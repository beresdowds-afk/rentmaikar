...
BEGIN
  ...
  VALUES (owner_id, 'TestMake', 'TestModel', 2024, 'TG-' || substr(gen_random_uuid()::text,1,10))
  ...
  VALUES (driver_id, owner_id, vehicle_id, 'rental', 1, 'ORIGINAL', 'pending', true)
  ...
  VALUES (vehicle_id, owner_id, 20000, 2000, 500, 36, 'USD', true, false, 'active', true)
  ...
  VALUES (driver_id, owner_id, vehicle_id, listing_id, 'ORIGINAL', 20000, 2000, 500, 36, 'USD', 'pending_signatures')
  ...
  VALUES (driver_id, owner_id, vehicle_id, 55, 'USD', 'pending')
  ...
  VALUES (driver_id, owner_id, vehicle_id, 75, 'USD', 'active', now(), now() + interval '7 days', 'daily', 'USA')
  RETURNING id INTO rent_id;

  INSERT INTO public.rideshare_profile_submissions(driver_id, vehicle_id, week_start_date, status)
  VALUES (driver_id, vehicle_id, date_trunc('week', now())::date, 'submitted')
  ...
  VALUES (driver_id, owner_id, vehicle_id, 'accident', 'low', 'reported', 'TG Incident', 'fixture', now(), now())
  RETURNING id INTO inc_id;

  INSERT INTO public.weekly_inspection_reports(driver_id, owner_id, vehicle_id, week_start_date)
  VALUES (driver_id, owner_id, vehicle_id, date_trunc('week', now())::date)
  RETURNING id INTO wr_id;

  -- Test 1: legal_agreements
  PERFORM set_config('request.jwt.claims', json_build_object('sub', driver_id::text, 'role', 'authenticated')::text, true);
  ...
  ASSERT (SELECT status FROM public.legal_agreements WHERE id=la_id)='pending', 'legal_agreements.status must not change from driver';
  ASSERT (SELECT driver_signature FROM public.legal_agreements WHERE id=la_id)='drv-sig', 'legal_agreements.driver_signature must be updatable by driver';
  ...
  ASSERT denied_after=denied_before+1, 'legal_agreements: expected one denial log row';

  -- Test 2: rent_to_own_agreements
  ...
  ASSERT (SELECT total_price FROM public.rent_to_own_agreements WHERE id=rto_id)=20000, 'RTO.total_price must not change from driver';
  ASSERT (SELECT driver_signature FROM public.rent_to_own_agreements WHERE id=rto_id)='drv-sig', 'RTO.driver_signature must be updatable by driver';
  ...
  ASSERT denied_after=denied_before+1, 'rent_to_own_agreements: expected one denial log row';

  -- Test 3: price_negotiations
  ...
  ASSERT (SELECT status FROM public.price_negotiations WHERE id=pn_id)='pending', 'price_negotiations.status must not change from non-admin';
  ASSERT (SELECT final_daily_rate FROM public.price_negotiations WHERE id=pn_id) IS NULL, 'price_negotiations.final_daily_rate must not change from non-admin';
  ...
  ASSERT denied_after=denied_before+1, 'price_negotiations: expected one denial log row';
  ...
  ASSERT (SELECT daily_rate FROM public.rentals WHERE id=rent_id)=75, 'rentals.daily_rate must not change from driver';
  ...
  ASSERT denied_after=denied_before+1, 'rentals: expected one denial log row';

  -- Test 5: rideshare_profile_submissions
  ...
  ASSERT (SELECT status FROM public.rideshare_profile_submissions WHERE id=rs_id)='submitted', 'rideshare.status must not change from driver';
  ASSERT (SELECT admin_notes FROM public.rideshare_profile_submissions WHERE id=rs_id) IS NULL, 'rideshare.admin_notes must not change from driver';
  ...
  ASSERT denied_after=denied_before+1, 'rideshare: expected one denial log row';

  -- Test 6: vehicle_incidents
  ...
  UPDATE public.vehicle_incidents SET status='resolved', severity='high', resolved_by=driver_id WHERE id=inc_id;
  ASSERT (SELECT status FROM public.vehicle_incidents WHERE id=inc_id)='reported', 'vehicle_incidents.status must not change from driver';
  ASSERT (SELECT severity FROM public.vehicle_incidents WHERE id=inc_id)='low', 'vehicle_incidents.severity must not change from driver';
  ...
  ASSERT denied_after=denied_before+1, 'vehicle_incidents: expected one denial log row';

  -- Test 7: weekly_inspection_reports
  ...
  ASSERT (SELECT admin_decision FROM public.weekly_inspection_reports WHERE id=wr_id) IS NULL, 'weekly_reports.admin_decision must not change from driver';
  ASSERT (SELECT photo_front_view FROM public.weekly_inspection_reports WHERE id=wr_id)='https://cdn/front.jpg', 'weekly_reports.photo_front_view must be updatable by driver';
  ...
  ASSERT denied_after=denied_before+1, 'weekly_reports: expected one denial log row';
  ...
  ASSERT (SELECT status FROM public.legal_agreements WHERE id=la_id)='active', 'admin must change legal_agreements.status';
  ASSERT (SELECT total_price FROM public.rent_to_own_agreements WHERE id=rto_id)=25000, 'admin must change RTO.total_price';
  ASSERT (SELECT status FROM public.price_negotiations WHERE id=pn_id)='approved', 'admin must approve price_negotiations';
  ASSERT (SELECT daily_rate FROM public.rentals WHERE id=rent_id)=80, 'admin must change rentals.daily_rate';
  ASSERT (SELECT status FROM public.rideshare_profile_submissions WHERE id=rs_id)='approved', 'admin must approve rideshare_profile_submissions';
  ASSERT (SELECT status FROM public.vehicle_incidents WHERE id=inc_id)='resolved', 'admin must resolve vehicle_incidents';
  ASSERT (SELECT admin_decision FROM public.weekly_inspection_reports WHERE id=wr_id)='approved', 'admin must set weekly_reports.admin_decision';

  SELECT count(*) INTO denied_after FROM public.permission_denied_log;
  ASSERT denied_after=denied_before, 'admin updates must not write denial log rows';
...
EXCEPTION WHEN OTHERS THEN
  BEGIN
    DELETE FROM public.permission_denied_log WHERE target_row_id IN (COALESCE(la_id::text,''), COALESCE(rto_id::text,''), COALESCE(pn_id::text,''), COALESCE(rent_id::text,''), COALESCE(rs_id::text,''), COALESCE(inc_id::text,''), COALESCE(wr_id::text,''));
    DELETE FROM public.weekly_inspection_reports WHERE id=wr_id;
    DELETE FROM public.vehicle_incidents WHERE id=inc_id;
    DELETE FROM public.rideshare_profile_submissions WHERE id=rs_id;
    DELETE FROM public.rentals WHERE id=rent_id;
